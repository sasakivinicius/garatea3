...
public:
  ...
  */
  void mindelay(uint32_t delayy, uint8_t nada){
    delay(8);
  }
  ...
  uint8_t readbyte(uint16_t pos){				//le um unico byte na posição especificada, sem restrições
    if(pos < memorysize){						//verifica se a posição ta dentro da memoria
            mindelay(operationdelay, lastcall);	//aguarda o tempo necessario pra dar o define "operationdelay", caso necessario
      temppos.full = pos;						//atualiza a posição temporaria de escrita para o byte
      Wire.beginTransmission(0b1010000);		//daqui pra baixo n sei mais o q ta acontecendo
      Wire.write(temppos.half[1]);
      Wire.write(temppos.half[0]);
      Wire.endTransmission();
  ...
  void writestring(char *data){					//escreve uma string na eeprom
    ...
    while(temcoisa){							//escrevendo a string
      ...
      Wire.beginTransmission(0b1010000);		//chama a biblioteca wire
  ...
  void readstring(uint16_t pos, char *bufferr){		//escreve uma string na eeprom
    uint16_t offset=0;
    uint8_t temcoisa=1;
    temppos.full=pos;
    while(temcoisa){
      ...
      Wire.write(temppos.half[0]);
      Wire.endTransmission();
      Wire.requestFrom(0b1010000,64 - max((memcount + temppos.full) % 64, 32));
      while(1){
...
void setup() {
  ...
  //char otherstring[240]="Essa outra string aqui tem um monte de letra so pra tentar sobreescrever aquela string que tava aqui antes de tudo isso acontecer mas que acabou acontecendo infelizmente e eu ja nem sei mais o que eu to digitando nesse trecho de texto";
  char otherstring[240]="1223334444555556666667777777888888889999999990000000000123456789";
  uint8_t aux2=240;
  char zipstr[121], unzipstr[121];
  char str[240];
  /*
  while(aux2!=0){
    otherstring[aux2]=aux2;
    aux2--;
  }*/
  eeprom.zipstring(otherstring,zipstr);
  uint16_t aux = 0;
  char c[280];
  uint8_t flag;
  delay(500);
  ...
  eeprom.writestring(zipstr);
  
  delay(1000);
  eeprom.readstring(3,unzipstr);
  eeprom.unzipstring(unzipstr,str);
  while(aux != 240){
